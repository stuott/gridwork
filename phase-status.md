...
This was checked against a real fog puzzle the same day — "Fogs-n-Dots-n-Knights" by Meggen033, which starts with only its centre box uncovered and no given digits at all. It loads and behaves correctly: the right nine cells are visible at the start, a correct digit uncovers the square around it, a wrong one does nothing. That puzzle also turned up two small unrelated annoyances, now fixed — the app was listing f-puzzles' internal editor settings among the "rules I'm not checking", including one that was empty.

The remaining unknown is the other half of the format: puzzles that light a 3×3 area from the start, and fog written in SudokuPad's own native format, haven't been seen in a real file yet. Those parts still come from reading another player's source code. If a fog puzzle ever loads with the wrong area uncovered, that's where to look.
